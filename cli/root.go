// Package cli builds the kr36 command tree on top of the kr36 library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: kr36.DefaultConfig()}

	root := &cobra.Command{
		Use:   "kr36",
		Short: "Browse 36kr Chinese tech news via RSS (36氪)",
		Long: `kr36 reads 36氪 (36kr.com) through its public RSS feed.
No API key is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

kr36 is an independent tool and is not affiliated with 36kr.`,
		SilenceUsage:  true,
		SilenceErrors: true,
