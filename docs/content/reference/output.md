...
describes what users get. Pick a format with `-o`, or let kr36 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kr36 <command> -o table   # aligned columns for reading
kr36 <command> -o jsonl   # one JSON object per line, for piping
kr36 <command> -o json    # a single JSON array
kr36 <command> -o csv     # spreadsheet friendly
kr36 <command> -o tsv     # tab-separated
kr36 <command> -o url     # just the URL column
kr36 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kr36 <command> --fields id,title,url
```
...
```bash
kr36 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kr36 <command>            # a table, because this is a terminal
kr36 <command> | wc -l    # JSONL, because this is a pipe
```
